...
# Prefer the explicit version of virtualenv, but some systems only have a
# non-numbered command only.
VIRTUALENV ?= virtualenv$(if $(shell which virtualenv$(PYTHON_MAJOR) 2> /dev/null),$(PYTHON_MAJOR),)
...
# pynodegl-install is in dependency to prevent from trying to install pynodegl from its requirements
...
# against, you can use your own link post-install:
#
#     % unlink sxplayer
